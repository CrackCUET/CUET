"""Pre-generate and pin Business Studies mocks (AI-generated questions) for reuse.

Usage:
  cd /app/backend && python scripts/pregenerate_business_studies_mocks.py --count 20

Notes:
- Uses GEMINI_API_KEY (Google GenAI SDK) if present via QuestionGenerator.
- Creates/ensures mocks titled "Business Studies - Mock Test N" for N=1..count
...
async def main(count: int):
    ...
    for i in range(1, count + 1):
        title = f"Business Studies - Mock Test {i}"
        mock = db.mock_tests.find_one({"subject": "Business Studies", "title": title}, {"_id": 0})
        if not mock:
            m = MockTest(
                subject="Business Studies",
                title=title,
                description=f"Full-length Business Studies mock test with 50 questions (Set {i})",
                total_questions=50,
                total_marks=250,
                duration_minutes=60,
    ...
    for idx, mock in enumerate(mocks, start=1):
        ...
        try:
            questions = []
            # Generate in smaller chunks for reliability
            for _ in range(5):
                batch = await generator.generate_mock_test_questions(subject="Business Studies", total_questions=10)
                questions.extend(batch)

            # Ensure we have enough unique questions to pin 50
            unique = []
            seen = set()
            for q in questions:
            ...
            for q in questions:
                ...
                existing = db.questions.find_one(
                    {"question_text": q.question_text, "subject": "Business Studies"},
                    {"_id": 0, "id": 1},
                )
                if existing:
                    qid = existing["id"]
