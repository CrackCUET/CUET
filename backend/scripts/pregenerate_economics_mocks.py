"""Pre-generate and pin Economics mocks (AI-generated questions) for reuse.

Usage:
  cd /app/backend && python scripts/pregenerate_economics_mocks.py --count 20

Notes:
- Uses GEMINI_API_KEY (Google GenAI SDK) if present via QuestionGenerator.
- Creates/ensures mocks titled "Economics - Mock Test N" for N=1..count
...
async def main(count: int):
    ...
    for i in range(1, count + 1):
        title = f"Economics - Mock Test {i}"
        mock = db.mock_tests.find_one({"subject": "Economics", "title": title}, {"_id": 0})
        if not mock:
            m = MockTest(
                subject="Economics",
                title=title,
                description=f"Full-length Economics mock test with 50 questions (Set {i})",
                total_questions=50,
                total_marks=250,
                duration_minutes=60,
    ...
    for idx, mock in enumerate(mocks, start=1):
        ...
        try:
            ...
            for _ in range(5):
                batch = await generator.generate_mock_test_questions(subject="Economics", total_questions=10)
            ...
            for q in questions:
                ...
                existing = db.questions.find_one(
                    {"question_text": q.question_text, "subject": "Economics"},
                    {"_id": 0, "id": 1},
                )
                if existing:
                    qid = existing["id"]
