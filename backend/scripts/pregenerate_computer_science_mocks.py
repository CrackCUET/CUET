"""Pre-generate and pin Computer Science mocks (AI-generated questions) for reuse.

Usage:
  cd /app/backend && python scripts/pregenerate_computer_science_mocks.py --count 20
"""
...
async def main(count: int):
    ...
    for i in range(1, count + 1):
        title = f"Computer Science - Mock Test {i}"
        mock = db.mock_tests.find_one({"subject": "Computer Science", "title": title}, {"_id": 0})
        if not mock:
            m = MockTest(
                subject="Computer Science",
                title=title,
                description=f"Full-length Computer Science mock test with 50 questions (Set {i})",
                total_questions=50,
                total_marks=250,
                duration_minutes=60,
    ...
    for idx, mock in enumerate(mocks, start=1):
        ...
        try:
            questions = []
            for _ in range(5):
                batch = await generator.generate_mock_test_questions(subject="Computer Science", total_questions=10)
            ...
            for q in questions:
                ...
                existing = db.questions.find_one(
                    {"question_text": q.question_text, "subject": "Computer Science"},
                    {"_id": 0, "id": 1},
                )
                if existing:
                    qid = existing["id"]
