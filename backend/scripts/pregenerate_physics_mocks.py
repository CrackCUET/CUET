"""Pre-generate and pin Physics mocks (AI-generated questions) for reuse.

Usage (recommended):
  cd /app/backend && python scripts/pregenerate_physics_mocks.py --count 30

Notes:
- Uses GEMINI_API_KEY (Google GenAI SDK) if present via QuestionGenerator.
- Creates/ensures mocks titled "Physics - Mock Test N" for N=1..count
...
# Ensure /app/backend is on sys.path when running as a script
...
async def main(count: int):
    ...
    for i in range(1, count + 1):
        title = f"Physics - Mock Test {i}"
        mock = db.mock_tests.find_one({"subject": "Physics", "title": title}, {"_id": 0})
        if not mock:
            m = MockTest(
                subject="Physics",
                title=title,
                description=f"Full-length Physics mock test with 50 questions (Set {i})",
                total_questions=50,
                total_marks=250,
                duration_minutes=60,
    ...
    for idx, mock in enumerate(mocks, start=1):
        ...
        try:
            questions = await generator.generate_mock_test_questions(subject="Physics", total_questions=50)
            if len(questions) < 50:
            ...
            for q in questions:
                ...
                existing = db.questions.find_one(
                    {"question_text": q.question_text, "subject": "Physics"},
                    {"_id": 0, "id": 1},
                )
                if existing:
                    qid = existing["id"]
