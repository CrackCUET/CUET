"""Pre-generate and pin History mocks (AI-generated questions) for reuse.

Usage:
  cd /app/backend && python scripts/pregenerate_history_mocks.py --count 20

Notes:
- Uses GEMINI_API_KEY (Google GenAI SDK) if present via QuestionGenerator.
- Creates/ensures mocks titled "History - Mock Test N" for N=1..count
...
async def main(count: int):
    ...
    for i in range(1, count + 1):
        title = f"History - Mock Test {i}"
        mock = db.mock_tests.find_one({"subject": "History", "title": title}, {"_id": 0})
        if not mock:
            m = MockTest(
                subject="History",
                title=title,
                description=f"Full-length History mock test with 50 questions (Set {i})",
                total_questions=50,
                total_marks=250,
                duration_minutes=60,
    ...
    for idx, mock in enumerate(mocks, start=1):
        ...
        try:
            questions = await generator.generate_mock_test_questions(subject="History", total_questions=50)
            if len(questions) < 50:
            ...
            for q in questions:
                ...
                existing = db.questions.find_one(
                    {"question_text": q.question_text, "subject": "History"},
                    {"_id": 0, "id": 1},
                )
                if existing:
                    qid = existing["id"]
